...
router.post("/add", (req, res) => {
  ...
  const sql = `
    INSERT INTO homecontentslider (icon, link, title, info)
    VALUES (?, ?, ?, ?)
  `;
...
// READ ALL
router.get("/", (req, res) => {
  db.query("SELECT * FROM homecontentslider", (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
...
router.get("/:id", (req, res) => {
  const { id } = req.params;
  db.query(
    "SELECT * FROM homecontentslider WHERE id = ?",
    [id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
      if (result.length === 0)
...
router.delete("/:id", (req, res) => {
  ...
  db.query("DELETE FROM homecontentslider WHERE id = ?", [id], (err) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json({
      success: true,
