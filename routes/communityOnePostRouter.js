...
router.post("/", upload.single("icon"), (req, res) => {
  ...
  const query = `
    INSERT INTO community_post (category_id, icon, title, text, path)
    VALUES (?, ?, ?, ?, ?)
  `;
  db.query(
    query,
...
router.get("/category/:category_id", (req, res) => {
  db.query(
    "SELECT * FROM community_post WHERE category_id = ? ORDER BY id DESC",
...
router.put("/:id", upload.single("icon"), (req, res) => {
  ...
  let query = "UPDATE community_post SET title = ?, text = ?, path = ?";
