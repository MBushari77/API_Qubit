...
router.post("/", upload.single("icon"), (req, res) => {
  ...
  const query = `
    INSERT INTO project_crew (category_id, icon, title, text, path)
    VALUES (?, ?, ?, ?, ?)
  `;
...
router.get("/", (req, res) => {
  db.query("SELECT * FROM project_crew ORDER BY id DESC", (err, rows) => {
    if (err) return res.status(500).json({ error: err.sqlMessage || err });
    res.json(rows);
  });
});
...
router.get("/category/:category_id", (req, res) => {
  db.query(
    "SELECT * FROM project_crew WHERE category_id = ? ORDER BY id DESC",
...
router.get("/:id", (req, res) => {
  db.query(
    "SELECT * FROM project_crew WHERE id = ?",
    [req.params.id],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.sqlMessage || err });
...
router.put("/:id", upload.single("icon"), (req, res) => {
  ...
  let query = "UPDATE project_crew SET title = ?, text = ?, path = ?";
...
router.delete("/:id", (req, res) => {
  db.query(
    "DELETE FROM project_crew WHERE id = ?",
    [req.params.id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.sqlMessage || err });
      if (result.affectedRows === 0)
