...
router.post("/", upload.single("icon"), (req, res) => {
  ...
  const query = `
    INSERT INTO product_section_five (category_id, icon, title, text, path)
    VALUES (?, ?, ?, ?, ?)
  `;
...
router.get("/category/:category_id", (req, res) => {
  db.query(
    "SELECT * FROM product_section_five WHERE category_id = ? ORDER BY id DESC",
...
router.get("/:id", (req, res) => {
  db.query(
    "SELECT * FROM product_section_five WHERE id = ?",
    [req.params.id],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.sqlMessage || err });
...
router.put("/:id", upload.single("icon"), (req, res) => {
  ...
  let query = "UPDATE product_section_five SET title = ?, text = ?, path = ?";
...
router.delete("/:id", (req, res) => {
  db.query(
    "DELETE FROM product_section_five WHERE id = ?",
    [req.params.id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.sqlMessage || err });
      if (result.affectedRows === 0)
