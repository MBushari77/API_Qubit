...
router.post("/", upload.single("icon"), (req, res) => {
  ...
  const query = `
    INSERT INTO sub_category_section_zero (category_id, icon, title, text, path)
    VALUES (?, ?, ?, ?, ?)
  `;
  db.query(
    query,
...
router.get("/category/:category_id", (req, res) => {
  db.query(
    "SELECT * FROM sub_category_section_zero WHERE category_id = ? ORDER BY id DESC",
...
router.get("/:id", (req, res) => {
  db.query(
    "SELECT * FROM sub_category_section_zero WHERE id = ?",
    [req.params.id],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.sqlMessage || err });
...
router.put("/:id", upload.single("icon"), (req, res) => {
  ...
  let query =
    "UPDATE sub_category_section_zero SET title = ?, text = ?, path = ?";
...
router.delete("/:id", (req, res) => {
  db.query(
    "DELETE FROM sub_category_section_zero WHERE id = ?",
    [req.params.id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.sqlMessage || err });
      if (result.affectedRows === 0)
