...
router.post("/add", upload.single("icon"), (req, res) => {
  ...
  const sql = "INSERT INTO community_blogs (name, icon) VALUES (?, ?)";
...
/** READ - Get all community_blogs */
router.get("/", (req, res) => {
  db.query("SELECT * FROM community_blogs", (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(results);
  });
});
...
router.get("/:id", (req, res) => {
  db.query(
    "SELECT * FROM community_blogs WHERE id = ?",
    [req.params.id],
    (err, results) => {
      if (err) return res.status(500).json({ error: err.message });
      if (results.length === 0)
...
router.put("/:id", upload.single("icon"), (req, res) => {
  ...
  db.query(
    "SELECT icon FROM community_blogs WHERE id = ?",
    [req.params.id],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.message });
      if (rows.length === 0)
      ...
      const sql = "UPDATE community_blogs SET name = ?, icon = ? WHERE id = ?";
...
router.delete("/delete/:id", (req, res) => {
  db.query(
    ...
    (err, rows) => {
      ...
      db.query(
        "DELETE FROM community_blogs WHERE id = ?",
        [req.params.id],
        (err) => {
          if (err) return res.status(500).json({ error: err.message });
