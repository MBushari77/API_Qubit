...
router.post("/add", upload.single("image"), (req, res) => {
  ...
  const sql = `
    INSERT INTO homecardsslider (image, active, link, title, info)
    VALUES (?, ?, ?, ?, ?)
  `;
...
router.get("/:id", (req, res) => {
  const { id } = req.params;
  db.query(
    "SELECT * FROM homecardsslider WHERE id = ?",
    [id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
      if (result.length === 0)
...
router.get("", (req, res) => {
  db.query("SELECT * FROM homecardsslider", (err, results) => {
    if (err) return res.status(500).json({ error: err.message });
...
router.put("/:id", upload.single("image"), (req, res) => {
  ...
  db.query(
    "SELECT image FROM homecardsslider WHERE id = ?",
    [id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
      if (result.length === 0)
      ...
      const sql = `
      UPDATE homecardsslider SET image = ?, active = ?, link = ?, title = ?, info = ?
      WHERE id = ?
    `;
      db.query(
        sql,
...
router.delete("/:id", (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      const sql = "DELETE FROM homecardsslider WHERE id = ?";
...
router.delete("", (req, res) => {
  ...
  const selectSql = `SELECT image FROM homecardsslider WHERE id IN (${placeholders})`;
  ...
  db.query(selectSql, ids, (selectErr, results) => {
    ...
    const deleteSql = `DELETE FROM homecardsslider WHERE id IN (${placeholders})`;
