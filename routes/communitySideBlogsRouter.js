...
router.post("/", upload.single("icon"), (req, res) => {
  ...
  const query = `
    INSERT INTO community_side_blogs (category_id, icon, title, text, path)
    VALUES (?, ?, ?, ?, ?)
  `;
...
router.get("/", (req, res) => {
  db.query("SELECT * FROM community_side_blogs", (err, rows) => {
    if (err) return res.status(500).json({ error: err.sqlMessage || err });
    res.json(rows);
  });
});
...
router.get("/category/:category_id", (req, res) => {
  db.query(
    "SELECT * FROM community_side_blogs WHERE category_id = ?",
...
router.get("/:id", (req, res) => {
  db.query(
    "SELECT * FROM community_side_blogs WHERE id = ?",
    [req.params.id],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.sqlMessage || err });
...
router.put("/:id", upload.single("icon"), (req, res) => {
  ...
  let query = "UPDATE community_side_blogs SET title = ?, text = ?, path = ?";
...
router.delete("/:id", (req, res) => {
  db.query(
    "DELETE FROM community_side_blogs WHERE id = ?",
    [req.params.id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.sqlMessage || err });
      if (result.affectedRows === 0)
