...
router.post("/", upload.single("icon"), (req, res) => {
  ...
  const query = `
    INSERT INTO single_project (category_id, icon, title, subtitle, text, path)
    VALUES (?, ?, ?, ?, ?, ?)
  `;
  db.query(
    query,
...
// READ all items
router.get("/", (req, res) => {
  db.query("SELECT * FROM single_project", (err, rows) => {
    if (err) return res.status(500).json({ error: err.sqlMessage || err });
    res.json(rows);
  });
});
...
router.get("/category/:category_id", (req, res) => {
  db.query(
    "SELECT * FROM single_project WHERE category_id = ?",
...
router.get("/:id", (req, res) => {
  db.query(
    "SELECT * FROM single_project WHERE id = ?",
    [req.params.id],
    (err, rows) => {
      if (err) return res.status(500).json({ error: err.sqlMessage || err });
...
router.put("/:id", upload.single("icon"), (req, res) => {
  ...
  let query =
    "UPDATE single_project SET title = ?, subtitle = ?, text = ?, path = ?";
...
router.delete("/:id", (req, res) => {
  db.query(
    "DELETE FROM single_project WHERE id = ?",
    [req.params.id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.sqlMessage || err });
      if (result.affectedRows === 0)
